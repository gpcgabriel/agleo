...
def initialize_simulation(dataset_gml, satellites_json, num_users, num_satellites, scenario, algorithm):
    ...
    # Load topology
    t = ds.load_topology(ground_topology=dataset_gml, leo_topology=satellites_json, max_satellites=num_satellites)
...
def execute_pending_action():
    ...
    spinner_msg = (
        f"⚙️ Running Network Simulation: {pending['description']}..."
        if action == "run_simulation"
        else f"🔧 Applying physical network change: {pending['description']}..."
    )
    with st.spinner(spinner_msg):
        if action == "run_simulation":
            ...
            st.session_state["chat_messages"].append(
                {"role": "system", "content": f"Action executed successfully: {pending['description']}"}
            )
